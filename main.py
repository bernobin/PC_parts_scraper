...
def get_graka_endpoint(soup):
    links = soup.find_all('a')

    for link in links:
        this_url = str(link.get('href'))
        if '/Grafikkarten' in this_url:
            return this_url


def build_menu(soup):
    links = soup.find_all('a')#, {'class': 'directOnly'})
    menu = Menu()

    for link in links:
        tab = str(link.get('href'))
        #e_class = link.get('class', [])
...
def main():
    ...
    graka_ep = get_graka_endpoint(soup)
    menu = build_menu(soup)
    path = menu.navigate()

    r = requests_client.get_products_list(path)

    #r = requests_client.get_products_list(graka_ep, filters['Nvidia_GeForce_RTX_4070 SUPER'])
